...
import MusicService, {
  IMusicToken,
  ITrack,
} from "../business/musics/MusicService";
...
export default class Rooms {
  ...
  public async get(
    id: string
  ): Promise<(IRoom & { service: MusicService }) | null> {
  ...
  public async join(
    ...
    const hasAlreadyThisMember =
      room.members.findIndex(({ id }) => id === me?.clientId) !== -1;
  ...
  public async broadcast<T>(
    id: string,
    message: Omit<RoomEvents.IncomingMessage<T>, "date">
  ): Promise<void> {
    const room = await this.get(id);
    if (!room) return;
    ...
    const newHistory = [
      ...room.history,
      { ...message, date: new Date() } as RoomEvents.IncomingMessage,
    ];
